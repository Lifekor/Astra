...
class IntentAnalyzer:
    ...
    def get_semantic_relevance(
        ...
        ВАЖНО: Оценивай релевантность по СМЫСЛУ, а не по позиции в тексте.
        - Игнорируй предисловия и заголовки, если они не относятся к запросу
        - Ищи фрагменты, которые ДЕЙСТВИТЕЛЬНО отвечают на вопрос пользователя
        - Не давай высокие оценки фрагментам только потому, что они в начале документа
        - Фокусируйся на конкретном содержании, связанном с запросом
